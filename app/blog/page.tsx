...
export default async function BlogMainPage() {
  const posts = await getAllPosts(); // [{ slug, metadata }]
  return (
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                ...
                <time dateTime={post.metadata.date}>
                  {post.metadata.date ? new Date(post.metadata.date).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" }) : "No date"}
                </time>
              </div>
            </div>
