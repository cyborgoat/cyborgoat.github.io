  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts.filter(post => {
      ...
      return (
        ...
        (tags && tags.some((tag: string) => tag.toLowerCase().includes(q)))
      );
    });
  }, [search, posts]);
  ...
  return (
    <section className="container max-w-3xl py-8 mx-auto">
        ...
        {paginatedPosts.map((post) => (
          ...
          >
            {post.metadata.thumbnail && (
              <Link href={`/blog/${post.metadata.slug}`} className="block mb-4">
                <Image
                  src={post.metadata.thumbnail!}
                  alt={post.metadata.title ? `${post.metadata.title} thumbnail` : "Blog post thumbnail"}
                  width={1080}
                  height={480}
            ...
            <p className="mb-3 text-muted-foreground">{post.metadata.excerpt}</p>
      ...
      {pageCount > 1 && (
        <div className="flex justify-center mt-8 gap-2">
          ...
          {Array.from({ length: pageCount }, (_, i) => (
            <button
              key={i}
              className={`px-3 py-1 rounded-md border text-sm ${page === i + 1 ? 'bg-primary text-primary-foreground' : 'bg-background hover:bg-muted'}`}
              onClick={() => setPage(i + 1)}
            >
              {i + 1}
            </button>
      ...
      {filteredPosts.length === 0 && !search && (
        <div className="text-center text-muted-foreground mt-12">No blogs found.</div>
      )}
      {filteredPosts.length === 0 && search && (
        <div className="text-center text-muted-foreground mt-12">No blogs match your search.</div>
      )}
    </section>
  );
}
