...
export default function MarkdownRender({ content, className }: MarkdownRenderProps) {
  const components: Components = {
    code(
      ...
      return <code className="bg-gray-100 dark:bg-gray-800 rounded px-1 py-0.5 text-sm font-mono text-gray-900 dark:text-gray-200">{children}</code>;
    },
    a({ href, children }) {
    ...
    table({ children }) {
      return (
        <div className="overflow-x-auto my-6">
          <table className="min-w-full border border-gray-300 dark:border-gray-700 text-sm text-gray-900 dark:text-gray-200">{children}</table>
        </div>
      );
    },
    thead({ children }) {
    ...
    tbody({ children }) {
      return <tbody className="divide-y divide-gray-200 dark:divide-gray-700 text-gray-900 dark:text-gray-200">{children}</tbody>;
    },
    th({ children }) {
    ...
    h2({ children }) {
      return <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-200 mb-4 border-b-2 border-gray-900 dark:border-gray-200">{children}</h2>;
    },
    h3({ children }) {
